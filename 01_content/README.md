...
- **控制室**：默认提供一个总览配置；它不是固定首位项目，控制室本身可以创建、隐藏、归档、恢复或删除。项目卡片网格实时监控房间内有效项目成员的状态（工作中/待你决定/已完成），零轮询零 Token。
...
- **导入/安全**：导出格式为 `dsh-control-rooms-v1.json`，导入只覆盖控制室配置并重映射冲突 ID。浏览器里的 `window.__dshWorktable.controlRooms` 是由 localStorage 支持的类型化手动/调试 seam，并不是自动注册到 DeepSeek/Host 模型工具目录的能力；在提供官方 Client Tool surface 或显式 adapter 之前，DeepSeek 不能自动发现或直接调用这些命令。删除、批量移除和替换规则等破坏性操作需要确认，最近 100 条变更保留在本地审计。
